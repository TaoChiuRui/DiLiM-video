...
def _stream_rotation(video: Path) -> int:
    """Rotation in degrees from display-matrix side data (or legacy rotate tag).

    Rotated 16:9 camera/phone footage stores landscape width×height plus a 90°
    display-matrix flag; players auto-rotate to portrait. Return that angle so
    orientation checks reason about the *displayed* frame, not the stored one.
    """
    try:
        out = subprocess.run(
            ["ffprobe", "-v", "error", "-select_streams", "v:0",
...
def is_portrait_source(video: Path) -> bool:
    ...
    honoring rotation metadata so rotated landscape footage is detected."""
    try:
        out = subprocess.run(
            ["ffprobe", "-v", "error", "-select_streams", "v:0",
